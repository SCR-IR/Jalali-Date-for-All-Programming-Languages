...
while [ "$type" != "q" ]
do
    echo 'please enter type mof input date
    j : Jalali
    g : gregorian
    q : exit
date type:';
...
-------------------------'
    ...
    then
        ...
        echo "gregorian: $year-$month-$day";
        echo "jalali:    "${jalali[0]}/${jalali[1]}/${jalali[2]};
    else
        jalali_to_gregorian $year $month $day;
        echo "jalali:    $year/$month/$day";
        echo "gregorian: "${gregorian[0]}-${gregorian[1]}-${gregorian[2]};
    fi
    echo '-------------------------
'
done
